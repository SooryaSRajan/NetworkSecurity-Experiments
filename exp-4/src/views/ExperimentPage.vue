            ...
            <div class="row-3-content-space">
              <div class="step-1" v-if="step===1">
                <p>In this experiment, we will be combining active and passive attacks to retrieve the username and
                  password of a
                  user. You are the machine titled 'Hacker' in the network. Begin by intercepting a packet from a
                  channel
                  of your choice.</p>
                <br>
            ...
            <NetworkInteractionComponent ref="childComponentRef">
              ...
              <EndSystemComponent top="70%" left="5%" class="box" id="box3">
                <ServerComponent :ip-address="ipAddress[2]" :port1="portsClientC[0]" :port2="portsClientC[1]"
                                 ...
                                 :port-labels="computerCPortLabels"/>
              </EndSystemComponent>
              ...
              <EndSystemComponent top="70%" left="75%" class="box" id="box5" :package-info="serverPackages">
                <ServerComponent :ip-address="ipAddress[4]" :port1="portsServer[0]" :port2="portsServer[1]"
                                 :port3="portsServer[2]"
                                 :port4="portsServer[3]"
                                 :port-labels="computerDPortLabels"/>
              </EndSystemComponent>
...
export default {
  ...
  mounted() {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      randomPortNumber = Math.floor(Math.random() * 8000) + 1000;
      this.computerBPortLabels.push(randomPortNumber);
      randomPortNumber = Math.floor(Math.random() * 8000) + 1000;
      this.computerCPortLabels.push(randomPortNumber);
      randomPortNumber = Math.floor(Math.random() * 8000) + 1000;
      this.computerDPortLabels.push(randomPortNumber);
      randomPortNumber = Math.floor(Math.random() * 8000) + 1000;
      this.mitmPortLabels.push(randomPortNumber);
    }
  ...
  methods: {
    ...
    buttonClick() {
      if (this.step === 1) {
        ...
        this.channelHackerData.push(`hacker@unencryptedChannel > Opening ports ${this.mitmPortLabels[0]} and ${this.mitmPortLabels[1]} on hacker`);
